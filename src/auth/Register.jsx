...
const Register = () => {
  ...
  // useLayoutEffect(async () => {},[scheduleStore.classes]);
  useEffect(() => {
    scheduleStore.classes;
  }, []);
  ...
  const handleReg = async () => {
    ...
    const msvRegex = /^(?=.*[A-Za-z])(?=.*\d){10,}$/;
    // if (!msvRegex.test(regData.msv)) {
    //   Alert.alert("Invalid Pass", "Please enter a valid Student ID");
    //   return; // Exit function if email format is invalid
    // }
    // if (!passwordRegex.test(regData.password)) {
    //   Alert.alert("Invalid Pass", "Password must be at least 8 characters and contain uppercase letters and numbers");
    //   return; // Exit function if email format is invalid
    // }
    ...
    if (regData.password.length < 8) {
      Alert.alert('Form Invalid', 'Password should be at least 8 characters');
      return;
    }
    if (regData.msv.trim() === '') {
  ...
  return (
    <VStack flex={1} space={20} justifyContent={'center'} alignItems={'center'}>
      ...
      <RNPickerSelect
        ...
        onValueChange={(value) => setRegData({ ...regData, classes: { name: value } })}
